...
(define (filter p lst)
  (define (filter lst acc)
    (cond
      [(empty? lst) (reverse acc)]
      [(p (first lst)) (filter (rest lst) (cons (first lst) acc))]
      [else (filter (rest lst) acc)]
      )
    )
  (filter lst '())
  )
...
(define (filtra-par-lg lst)
  (define (pickOperation x)
    (cond
      [(integer? x) (even? x)]
      [else #f])
    )
  (filter pickOperation lst)
...
;;bonus
(define (tamanho-lista-strings-concat-first-join lstr)
  (string-length (string-join lstr ""))
  )
...
(time (tamanho-lista-strings-concat-first-join (bigStr 1000000)))
...
(define-test-suite test-tamanho-lista
  ...
  (test-equal? "strings vazias não alteram resultado"
               (tamanho-lista-strings (list "" "Tokyo" "Osaka" "" "Nara" ""))
               14)
  (test-equal? "lista vazia" (tamanho-lista-strings-concat-first-join '()) 0)
  (test-equal? "algumas strings"
               (tamanho-lista-strings-concat-first-join (list "Tokyo" "Osaka" "Nara"))
               14)
  (test-equal? "strings vazias não alteram resultado"
               (tamanho-lista-strings-concat-first-join (list "" "Tokyo" "Osaka" "" "Nara" ""))
               14)
...
;;
(define (todos? p lst) 
  (foldr (lambda (x y) (and (p x) y)) #t lst)
  )
...
(define (partition p lst)
  (define (partition lst acc1 acc2)
    (cond
      [(empty? lst) (list (reverse acc1) (reverse acc2))]
      [(p (first lst)) (partition (rest lst) (cons (first lst) acc1) acc2)]
      [else (partition (rest lst) acc1 (cons (first lst) acc2))]
      )
  )
  (partition lst '() '())
  )
...
(define (menor-quociente x lst)
  (define (mapFunction j)
    (/ j x))
  (define (foldFunction i k)
        (if (< i k)
            i
            k))
  (if (or (zero? x) (empty? lst)) "NaN" (foldl foldFunction +inf.0 (map mapFunction lst)))
  )
...
(define-test-suite test-menor-quociente
  (test-equal? "Lista vazia"
               (menor-quociente 3 '())
               "NaN")
  (test-equal? "Exemplo Base"
               (menor-quociente 3 '(1 2 3 4 5))
               1/3)
  (test-equal? "Divisão por zero"
               (menor-quociente 0 '(1 2 3 4 5))
               "NaN")
  (test-equal? "Lista Híbrida"
               (menor-quociente 3 (list 2.5 4/7 6 8))
               4/21))
