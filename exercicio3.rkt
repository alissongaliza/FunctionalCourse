...
;; funcionam ou não.
(define-test-suite test-remove-string
  (test-equal? "lista vazia primeiro"            (remove-primeiro "cinco" '())                                   '())
  (test-equal? "lista vazia todos"               (remove-todos    "cinco" '())                                   '())
  (test-equal? "uma ocorrência primeiro"         (remove-primeiro "cinco" '("um" "tres" "cinco" "sete"))         '("um" "tres" "sete"))
  (test-equal? "uma ocorrência todos"            (remove-todos    "cinco" '("um" "tres" "cinco" "sete"))         '("um" "tres" "sete"))
  (test-equal? "múltiplas ocorrências primeiro"  (remove-primeiro "cinco" '("um" "tres" "cinco" "sete" "cinco")) '("um" "tres" "sete" "cinco"))
  (test-equal? "múltiplas ocorrências todos"     (remove-todos    "cinco" '("um" "tres" "cinco" "sete" "cinco")) '("um" "tres" "sete"))
  (test-equal? "nenhuma ocorrência primeiro"     (remove-primeiro "tres"  '("onze" "sete" "treze" "doze"))       '("onze" "sete" "treze" "doze"))
  (test-equal? "nenhuma ocorrência todos"        (remove-todos    "tres"  '("onze" "sete" "treze" "doze"))       '("onze" "sete" "treze" "doze"))
  )
(define-test-suite test-remove-heterogeneo
  (test-equal? "lista vazia primeiro"            (remove-primeiro "cinco" '())                                   '())
  (test-equal? "lista vazia todos"               (remove-todos    "cinco" '())                                   '())
  (test-equal? "uma ocorrência primeiro"         (remove-primeiro "cinco" '(1 3 "cinco" 7))                      '(1 3 7))
  (test-equal? "uma ocorrência todos"            (remove-todos    "cinco" '(1 3 "cinco" 7))                      '(1 3 7))
  (test-equal? "múltiplas ocorrências primeiro"  (remove-primeiro "cinco" '(1 3 "cinco" 7 "cinco"))              '(1 3 7 "cinco"))
  (test-equal? "múltiplas ocorrências todos"     (remove-todos    "cinco" '(1 3 "cinco" 7 "cinco"))              '(1 3 7))
  (test-equal? "nenhuma ocorrência primeiro"     (remove-primeiro "tres"  '(11 7 23 "doze"))                     '(11 7 23 "doze"))
  (test-equal? "nenhuma ocorrência todos"        (remove-todos    "tres"  '(11 7 23 "doze"))                     '(11 7 23 "doze"))
  )
...
(define (remove-duplicatas lst)
  (cond
    [(empty? lst) '()]
    [else (cons (first lst) (remove-duplicatas (remove-todos (first lst) (rest lst))))]
  )
)
...
(define (uniao c1 c2)
  (cond
    ...
    [(empty? c2) c1]
    [(not(empty? c1)) (cons (first c1) (remove-todos (first c1) (uniao (rest c1) c2)))]
    [(empty? c1) c2]
  )
