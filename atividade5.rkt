...
;;
(define (map f lst)
  (define (map lst current acc)
    (cond
      [(empty? lst) (reverse acc)]
      [else (map (rest lst) (cons (first lst) current) (cons (f (first lst)) acc))]
      )
    )
  (map lst '() '())
  )
...
(define (tamanho-str-lista lista-strings)
  (map (lambda (x) (string-length x)) lista-strings)
  )
...
(define (adiciona-prefixo pre lista-assuntos) 
  (define (add-pre x)
    (if (equal? x "") (string-append "[" pre "]") (string-append "[" pre "] " x))
    )
  (map add-pre lista-assuntos)
  )
...
;;
(define (fold-right f start lst)
  (define (fold-right lst acc)
    (cond
      [(empty? lst) acc]
      [else (fold-right (rest lst) (f (first lst) acc))]
      )
    )
  (fold-right (reverse lst) start)
  )
...
(define (and-lista lst)
  (if (empty? lst) #f  (fold-right (lambda (x y) (and x y)) #t lst))
  )

(define (or-lista lst)
  (if (empty? lst) #t  (fold-right (lambda (x y) (or x y)) #f lst))
  )
...
;;
(define (fold-left f start lst)
  (define (fold-left lst acc)
    (cond
      [(empty? lst) acc]
      [else (fold-left (rest lst) (f acc (first lst) ))]
      )
    )
  (fold-left lst start)
  )
...
(define (pontuacao-final-fr ini lstded)
  (fold-right (lambda (x y) (- y x)) ini lstded)
  )
...
(define (concat-lista-sep lista-str sep)
  (define (concat-aux x y)
    (if (equal? y "") x (string-append x sep y))
    )
  (cond
    [(empty? lista-str) ""]
    [(equal? 1 (length lista-str)) (first lista-str)]
    [else (fold-right concat-aux "" lista-str)]
    )
  )
